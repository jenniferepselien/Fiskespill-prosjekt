...
class Hinder(pygame.sprite.Sprite):
    def __init__(self, x: int, y: int, image: pygame.surface.Surface, hinder_typ: pygame.surface.Surface):
    ...
    # Poeng 
        ...
        if self.hinder_typ == 'gress':
            if fisk_start_pos[0] > self.rect.topleft[0] and not self.passed:
                self.enter = True
            if fisk_start_pos[0] > self.rect.topright and not self.passed:
                self.exit = True
